...
mostlikely?''  Trying $r = 1,2,...,5$, one finds that $r = 4$ maximizes
...
\textbf{rectools}.  The functions to use are \textbf{trainMLE()} and
\textbf{predict.ydotsMLE()}.\footnote{For the Method of Moments in the
linear case, use \textbf{trainMM()} and predict.ydotsMM().}
