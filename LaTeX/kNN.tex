...
\subsubsection{Jaccuard Similarity}
...
$\widehat{a}_{ij}$ is the dot product of row $i$ in $W$ and column $j$
in $H$.  

Now consider what will happen with NC.  Assume for now that NC is a user
already in the database, row $r$.  We can now form a neighborhood of NC
by finding the $k$ most similar rows of $W$ to row $r$.  In particular,
let the item of interest to NC be item $s$.  Then for each of these rows
$m$, find $\widehat{a}_{ms}$, then average those values to obtain our
prediction for NC. 
